...
gps.vac  = {'v0', 'vi', 'vf'};         % vaccination group
...
bds(xi.vcm,:)               = repmat([-0.05, 0.05],9,1);                   % variation in the element of contact matrix

prm.bounds                  = bds';


% --- Set up the parameters (CFR)among age groups (<18y,18-60y,>60y) -----------------------------------------------
p.cfr          = [[0.0008, 0.0117, 0.1092],2.5*[0.0008, 0.0117, 0.1092]];    % Risk group-specific case fatality rates 

% --- Set up parameter values [vaccine effectiveness]
p.c1         = 0.6;  %0.6;  [choose as required]                             % Relative susceptibility of the vaccinated groups
p.c2         = 0;                                                          % Relative infectiousness of the vaccinated groups compared to others 
p.c3         = 0; %0.6  [choose as required]                             % Relative probability of developing symptomatic cases in the vaccinated groups

% Inter-age contact matrix 
prm.contact = [[1.35,1.43,0.04];[2.50,2.91,0.10];[0.28,0.34,0.02]];% %Age group(<18y,18-60y,>60y) 
p.imm       = 1;
...
% Age-group of Population [<18y,18-60y,>60y]
prm.N     = [0.365, 0.54, 0.095]*1e5;       
...
r.vacc = 0*ones(2,3);                          % rate of vaccination [r.vacc(ir,ia) This will be a 2X3 matrix]  
r.satu =  1/21;                                % Per-capita rate of vaccine induced immunity 

p.risk    = [0.028,0.143,0.43];                % Proportion having comorbidity among different age groups[0-18y, 18-60y, >60]   
